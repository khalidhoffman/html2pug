    ...
    for (let a = 0; a < node.attrs.length; a++) {
      ...
      switch (attr.name) {
        ...
        case 'class':
          ...
          _classList = attr.value.split(' ');
          _.forEach(_classList, function (className) {
            if (/^[\w\-]+$/.test(className)) {
              classList.push(className);
            } else {
              _unsafeClassList.push(className)
            }
          });
          attributes.push(`class="${_unsafeClassList.join(' ')}"`);
